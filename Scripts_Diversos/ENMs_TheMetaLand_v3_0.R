    ...
    especies.treino<-unique(ocorrencias.treino[,1])
      ...
      if (any(algoritmo=='maxentD')){
        ...
        for (h in 1:length(especies.treino)){
          ...
          Maxent.sp.default<-raster::predict(object=project,model=Maxent.modelo)
...
if (particao =="leave1out"){
  ...
  setwd(dir2)
  write.xlsx(ocorrencias.treino[,c(1:3)],paste("Ocorrencias_treino_",".xls",sep=""),row.names=F)
  write.xlsx(ocorrencias.teste[,c(1:3)],paste("Ocorrencias_teste_",".xls",sep=""),row.names=F)
...
ENMs_TheMetaLand('C:\\Poliquetas\\Poliquetas_Pacifico\\Rotated Pacific','asc',pca='N',project='N',10000,"bootstrap",algoritmo=c('MDA'))
