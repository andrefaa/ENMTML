ENMs_TheMetaLand('C:\\OneDrive\\SDM_Aedes\\VarInput','asc',pca='N',project='N',"bootstrap",algoritmo=c('svm.cls','svm.reg','maxentS'),msdm=2)
...
#3.3.Criar os modelo de distribuicao
  ...
  for (g in 1:length(especies.treino)){
    ...
    if(msdm==2|| msdm==3|| msdm==4){
      print(paste(dir.msdm,list.msdm[g],sep="\\"))
    ...
    if (any(algoritmo=='RF')){
      ...
      if (project=='S'){
        ...
        novdata<-as.matrix(env.svm[,-c(1,2)])
        RF.sp<-predict(object=RF.modelo,newdata=novdata,type="response")
        RF.xy<-data.frame(cbind(env.svm[,1:2],RF.sp))
        gridded(RF.xy)<-~x+y
        RF.raster<-raster(RF.xy)
