ENMs_TheMetaLand('C:\\OneDrive\\SDM_Aedes\\VarInput','asc',pca='N',project='N',"bootstrap",algoritmo=c('svm','maxentS','RF'),msdm=0)
...
ENMs_TheMetaLand<-function(diretorio,env='',pca='',project='',particao,algoritmo='',msdm){
 ...
 ## OBSERVACOES IMPORTANTES!!!!!!!!!!!!!!!:
  ...
  install_load(c("raster","sp","dismo","rJava","kernlab","maps","SDMTools","adehabitatHS","xlsx","randomForest","mda"))
...
#1.VARIAVEIS AMBIENTAIS----
  ...
  if(env == 'raster'){
    ...
    environmental.t<-stack(list.files(pattern='.bil'))
  ...
  #1.1.Realizar a PCA nas camadas ambientais(utilizar PCA como input)
    ...
    if (project=='S'){
      ...
      project.t<-raster(asc_proj)
      camadas.proj<-proj_prep[-1]
      
      for (a in 1:length(camadas.proj)){
        asc.p<-read.asc(camadas.proj[a])
        raster.p<-raster(asc.p)
        project.t<-stack(project.t,raster.p)
      }
      names(project.t)<-substr(proj_prep,1,nchar(proj_prep)-4)
      project.env<-rasterToPoints(project.t)
      ...
      scale<-scale %*% diag(1/stds)
        ...
        for (x in 1:ncol(project.env.raw)){
          ...
          gridded(PC)<- ~x+y
          PC<-raster(PC)
          project.t<-stack(project.t,PC)
          nomes.PC[x]<-paste("PC",x,sep="")
        }
      names(project.t)<-nomes.PC
      project.t<-project.t[[1:nlayers(environmental.t)]]
      project.env<-rasterToPoints(project.t)   
      }
    }else{
      project.t<-environmental.t
      project.env<-rasterToPoints(project.t)
    }
...
if (particao =="bootstrap"){
  ...
  if (msdm==1){
    print("Select the folder with X/Y M-SDM layer:")
    setwd(choose.dir())
    dir.smdm<-getwd()
    list.msdm<-list.files(pattern = ".asc")
    for (a in 1:length(list.msdm)){
      msdm.asc<-read.asc(list.msdm[a])
      msdm.ras<-raster(msdm.asc)
      environmental.t<-stack(environmental.t,msdm.ras)
    }
    environmental<-environmental.t
    project.t<-environmental.t
    project<-project.t
...
#3.3.Criar os modelo de distribuicao
  ...
  for (g in 1:length(especies.treino)){
    ...
    if(msdm==2|| msdm==3|| msdm==4){
      msdm.asc<-read.asc(paste(dir.smdm,list.msdm[g],sep="/"))
      msdm.ras<-raster(msdm.asc)
      environmental<-stack(environmental.t,msdm.ras)
      project<-stack(project.t,msdm.ras)
      project.env<-rasterToPoints(project)
    ...
    background.sample<-sample(1:nrow(project.env),length(pb.0))
    background.sample<-project.env[background.sample,]
    ...
    #Criar 10000 pseudo-aus?ncias para avalia??o do modelo
    #pseudo.aval<-as.data.frame(project.env[sample(1:nrow(project.env),10000),-c(1:2)])
    ...
    if (any(algoritmo=='bioclim')){
      ...
      #eval<-evaluate(ocorrencias.treino.sp[,-c(1:3)],pseudo.aval,Bioclim.modelo)
      #pos.ev<-which((eval@TPR+eval@TNR)==max(eval@TPR+eval@TNR))
      #spec<-eval@TNR[pos.ev]
      #sens<-eval@TPR[pos.ev]
      #TSS<-sens+spec-1
      #threshold(eval, stat='spec_sens', sensitivity=1)
    }
    ...
    if (any(algoritmo=='maxentS')){
      ...
      writeRaster(Maxent.sp,paste(especies.treino[g],'.asc',sep=''),format='ascii')
    ...
    if (any(algoritmo=='maxentD')){
      ...
      writeRaster(Maxent.sp.default,paste(especies.treino[g],'.asc',sep=''),format='ascii')
    ...
    if (any(algoritmo=='svm')){
      ...
      #svm.modelo2<-ksvm(pb1~.,data=input.occ.back,scaled=F,type="eps-svr")
      #SVM.sp2<-predict(project,svm.modelo2,type = "response")
      #plot(SVM.sp2)
      #SVM.sp3<-(SVM.sp2-minValue(SVM.sp2))/(maxValue(SVM.sp2)-minValue(SVM.sp2))
      #plot(SVM.sp3)
      
      #svms<-stack(SVM.sp,SVM.sp3)
      #svms2<-rasterToPoints(svms)
      #svms3<-cor(svms2[,3:4])
      
      writeRaster(SVM.sp,paste(especies.treino[g],'.asc',sep=''),format='ascii')
    ...
    if (any(algoritmo=='glm')){
      ...
      GLM.sp<-predict(project,glm.modelo,type = "response")
      writeRaster(GLM.sp,paste(especies.treino[g],'.asc',sep=''),format='ascii')
    ...
    if (any(algoritmo=='RF')){
      ...
      writeRaster(RF.raster,paste(especies.treino[g],'.asc',sep=''),format='ascii')
    ...
    if (any(algoritmo=='MDA')){
      ...
      writeRaster(MDA.sp,paste(especies.treino[g],'.asc',sep=''),format='ascii')
