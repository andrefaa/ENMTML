...
ENMs_TheMetaLand('C:\\Users\\Santiago\\Documents\\FORESTAL\\1-Trabajos\\43-MSDM\\2-Data\\M_SDM_MIN2017\\ASC',
                 env='asc',pca='N',project='N',Prev=0.5,particao="random",transf="N",
                 algoritmo=c("maxentS","glm","RF","svm.reg"),msdm=2)

data<-read.table(choose.files(),head=TRUE)
# data[1:450,1]<-paste("0",data[1:450,1],sep="")

ENMs_TheMetaLand<-function(diretorio,env='',pca='',project='',Prev,particao,transf,algoritmo='',msdm){
  ...
  #Prev:prevalencia do modelo (razao entre presencas e pseudo-ausencias) (0-1)
  ...
  #random : Particao aleatoria dos dados baseada em uma porcentagem
  #leave1out : Aconselhado para poucos dados(<5);cria os modelos com n-1 ocorrencia e testa na ocorrencia restante
  #transf: avaliar a transferabilidade dos modelos
  #lat.band : avaliar a transferabilidade por meio de bandas latitudinais
  #checker : avaliar a transferabilidade por quadriculas
  ...
  source("C:\\Scripts_for_ENM_TheMetaLand\\PCA_env_TML.R")
  source("C:\\Scripts_for_ENM_TheMetaLand\\Optimal_Quadrants_Selection_TML.R")
  source("C:\\Scripts_for_ENM_TheMetaLand\\Occ_Unicas_TML.R")
  source("C:\\Scripts_for_ENM_TheMetaLand\\Random_Moran_e_Schoener_TML.R")
  source("C:\\Scripts_for_ENM_TheMetaLand\\niche.overlap.functions.R")
  source("C:\\Scripts_for_ENM_TheMetaLand\\occ.prep.functions.R")
  source("C:\\Scripts_for_ENM_TheMetaLand\\Moran_for_Quadrants.R")
  
  
  # Instalar e carregar os pacotes
  #install.packages("install.load")
  library(install.load)
  install_load(c("raster","sp","dismo","rJava","kernlab","maps","SDMTools","adehabitatHS","xlsx","randomForest","mda","rgdal","dummies","ape","gbm","MASS","biomod2","ade4","adehabitat","gam","mvtnorm"))
  ...
  if (pca=="S"){
    environmental.t<-PCA_env_TML(environmental.t)
  }
  ...
  dir2<-"Result"
  setwd("..")
  if (file.exists(paste(getwd(),dir2,sep="/"))==F){
    dir.create(file.path(getwd(), dir2))
    dir2<-file.path(getwd(), dir2)
  }else{
    dir2<-file.path(getwd(), dir2)
  }
  ...
  ocorrencias.unicas<-Occ_Unicas_TML(environmental.t,ocorrencias)
  ...
  #3.1.Transferabilidade----
  if(transf=="lat.band"){
    
    dir.transf<-"ModelosTransf_Lat"
    setwd(diretorio)
    setwd("..")
    if (file.exists(paste(getwd(),dir.transf,sep="/"))==F){
      dir.create(file.path(getwd(), dir.transf))
      dir.transf<-file.path(getwd(), dir.transf)
    }else{
      dir.transf<-file.path(getwd(), dir.transf)
    }
    
    dir.out<-"Optimal"
    if (file.exists(file.path(diretorio,dir.out))){
      dir.out<-file.path(diretorio,dir.out)
    } else {
      dir.create(file.path(diretorio,dir.out))
      dir.out<-file.path(diretorio,dir.out)
    }
    
    setwd(diretorio)
    
    if(transf=="lat.band"&&particao!="random"){
      warning("Partition automatically adjusted to random!")
      particao<-"random"
    }
    
    if(any(grepl("pca",names(environmental.t)))==T){
      pca<-"S"
    }
    
    if(length(list.files(dir.out)) !=0){
      dir.opt<-dir.out
    }else{
      if(pca=="N"){
        transf.env<-PCA_env_TML(environmental.t)
        transf.env<-transf.env[[c(1,2)]]
      }else{
        transf.env<-environmental.t[[c(1,2)]]
      }
      dir.opt<-Optimal_Quadrants_Selection_TML(transf.env,ocorrencias.unicas)
    }
    occ.transf.aval<-NULL
    
    for (x in especies){
      occ.transf<-ocorrencias.unicas[ocorrencias.unicas[,1]==x,]
      ext1<-raster(read.asc(paste(dir.opt,list.files(dir.opt,pattern=x)[1],sep="\\")))
      ext2<-raster(read.asc(paste(dir.opt,list.files(dir.opt,pattern=x)[2],sep="\\")))
      ext<-list(EVEN=ext1,ODD=ext2)
      rm(ext1,ext2)
      for (y in 1:2){
        ext.t<-ext
        names(ext.t[[y]])<-names(ext)[[y]]
        names(ext.t[[-y]])<-names(ext)[[y]]
        transf.msk<-stack(mask(environmental.t,ext[[y]]),ext[[y]])
        transf.msk.env<-rasterToPoints(transf.msk)
        proj.msk<-stack(mask(environmental.t,ext[[-y]]),ext[[-y]])
        occ.transf.msk<-na.omit(cbind(occ.transf[,1:3],as.data.frame(extract(transf.msk,occ.transf[,2:3]))))
        names(ext.t[[-y]])<-names(ext)[[-y]]
        
        pb1<-rep(1,nrow(occ.transf.msk))
        ocorrencias.treino.max<-cbind(occ.transf.msk,pb1)
        species.pb<-rep(especies[x],nrow(ocorrencias.treino.max))
        pb.0<-rep(0,round(((nrow(occ.transf.msk)-(Prev*nrow(occ.transf.msk)))/Prev)))
        background.sample<-sample(1:nrow(transf.msk.env),length(pb.0))
        background.sample<-transf.msk.env[background.sample,]
        ...
        input.occ.back<-na.omit(input.occ.back)
        row.names(input.occ.back)<-NULL
        
        if(any(algoritmo=="maxentS")){
          
          dir.mxt<-"MXS"
          if (file.exists(file.path(dir.transf, dir.mxt))){
            dir.mxt<-file.path(dir.transf, dir.mxt)
          } else {
            dir.create(file.path(dir.transf, dir.mxt))
            dir.mxt<-file.path(dir.transf, dir.mxt)
          }
          
          Maxent.transf<-maxent(x=transf.msk,p=occ.transf.msk[,c(2:3)],args=c(
            "randomseed","noproduct","nothreshold","nohinge","noautofeature","nodoclamp","maximumiterations=1000"))
          
          Maxent.transf<-raster::predict(object=proj.msk,model=Maxent.transf)
          crs(Maxent.transf)<-crs(environmental.t)
          writeRaster(Maxent.transf,paste(dir.mxt,paste(x,names(ext.t[[y]]),"-",names(ext.t[[-y]]),'.asc',sep=''),sep="/"),format='ascii',overwrite=T)
        }
        
        if (any(algoritmo=='svm.reg')){
          
          dir.svt<-"SVR"
          if (file.exists(file.path(dir.transf, dir.svt))){
            dir.svt<-file.path(dir.transf, dir.svt)
          } else {
            dir.create(file.path(dir.transf, dir.svt))
            dir.svt<-file.path(dir.transf, dir.svt)
          }
          
          svm.modelo<-ksvm(pb1~.,data=input.occ.back,scaled=F,type="eps-svr")
          SVM.sp<-predict(proj.msk,svm.modelo,type = "response")
          SVM.sp<-(SVM.sp-minValue(SVM.sp))/(maxValue(SVM.sp)-minValue(SVM.sp))
          crs(SVM.sp)<-crs(environmental.t)
          writeRaster(SVM.sp,paste(dir.svt,paste(x,names(ext.t[[y]]),"-",names(ext.t[[-y]]),'.asc',sep=''),sep="/"),format='ascii',overwrite=T)
        }
        
        if (any(algoritmo=='svm.cls')){
          
          dir.sct<-"SVC"
          if (file.exists(file.path(dir.transf, dir.sct))){
            dir.sct<-file.path(dir.transf, dir.sct)
          } else {
            dir.create(file.path(dir.transf, dir.sct))
            dir.sct<-file.path(dir.transf, dir.sct)
          }
          
          svm.modelo<-ksvm(pb1~.,data=input.occ.back,scaled=F,type="C-svc",prob.model=T)
          env.temp<-as.matrix(proj.msk)
          c<-1:nrow(env.temp)
          env.temp<-cbind(c,env.temp)
          c<-env.temp[is.na(env.temp[,2])==F,1]
          d<-rep(NA,nrow(env.temp))
          novdata<-as.matrix(rasterToPoints(proj.msk)[,-c(1,2)])
          SVM.sp<-kernlab::predict(object=svm.modelo,newdata=novdata,type="probabilities")
          d[c]<-SVM.sp[,2]
          xy<-xyFromCell(proj.msk[[1]], 1:ncell(proj.msk[[1]]) )
          SVM.sp<-as.data.frame(cbind(xy,d))
          colnames(SVM.sp)<-c("x","y","Adeq")
          gridded(SVM.sp)<-~ x+y
          SVM.sp<-raster(SVM.sp)
          crs(SVM.sp)<-crs(environmental.t)
          writeRaster(SVM.sp,paste(dir.sct,paste(x,names(ext.t[[y]]),"-",names(ext.t[[-y]]),'.asc',sep=''),sep="/"),format='ascii',overwrite=T)
        }
        
        if (any(algoritmo=='RF')){
          
          dir.rft<-"RDF"
          if (file.exists(file.path(dir.transf, dir.rft))){
            sdir.rtf<-file.path(dir.transf, dir.rft)
          } else {
            dir.create(file.path(dir.transf, dir.rft))
            dir.rtf<-file.path(dir.transf, dir.rft)
          }
          ...
          RF.sp<-predict(proj.msk,RF.modelo)
          crs(RF.sp)<-crs(environmental.t)
          writeRaster(RF.sp,paste(dir.rtf,paste(x,names(ext.t[[y]]),"-",names(ext.t[[-y]]),'.asc',sep=''),sep="/"),format='ascii',overwrite=T)
        }
        
        occ.transf.msk[,1]<-paste(x,names(ext.t[[-y]]),"-",names(ext.t[[y]]),sep="")
        occ.transf.aval<-rbind(occ.transf.aval,occ.transf.msk)
      }
    }
    write.xlsx(occ.transf.aval,paste(dir.transf,"Ocorrencias Avaliacao.xls",sep="/"),row.names=F,sheetName = "Sheet1")
  }
  
  if(transf=="random"){
    print("Choose directory with transferability latitudinal bands to be used as mask")
    dir.msk<-choose.dir(getwd())
  }
  ...
  if (particao =="random"){
    ...
    for (y in 1:replicas){
      ...
      ocorrencias.treino.rep<-NULL
      ocorrencias.teste.rep<-NULL
      ...
      if (porcentagem==1){
        for (x in 1:length(especies)){
          ...
          if (nrow(ocorrencias.unicas.especie)<5){
            ...
            ocorrencias.treino.rep<-rbind(ocorrencias.treino.rep,occ.treino.sp)
          }
        }
        setwd(dir2)
        if (replicas==1){
          ocorrencias.treino<-ocorrencias.treino.rep
          write.xlsx(ocorrencias.treino[,c(1:3)],paste("Ocorrencias_treino",".xls",sep=""),row.names=F)    
        }else{
          ocorrencias.treino.rep[,1]<-paste(ocorrencias.treino.rep[,1],y,sep="")
          ocorrencias.treino<-rbind(ocorrencias.treino,ocorrencias.treino.rep)
        }
        especies.treino<-unique(ocorrencias.treino.rep[,1])
        ...
        for (x in 1:length(especies)){
          ...
          if (nrow(ocorrencias.unicas.especie)<5){
            ...
            ocorrencias.treino.rep<-rbind(ocorrencias.treino.rep,occ.treino.sp)
            ocorrencias.teste.rep<-rbind(ocorrencias.teste.rep,occ.teste.sp)
          }
        }
        setwd(dir2)
        if (replicas==1){
          ocorrencias.treino<-ocorrencias.treino.rep
          ocorrencias.teste<-ocorrencias.teste.rep
          write.xlsx(ocorrencias.treino[,c(1:3)],paste("Ocorrencias_treino",".xls",sep=""),row.names=F)
          write.xlsx(ocorrencias.teste[,c(1:3)],paste("Ocorrencias_teste",".xls",sep=""),row.names=F)
        }else{
          ocorrencias.treino.rep[,1]<-paste(ocorrencias.treino.rep[,1],y,sep="")
          ocorrencias.teste.rep[,1]<-paste(ocorrencias.teste.rep[,1],y,sep="")
          ocorrencias.treino<-rbind(ocorrencias.treino,ocorrencias.treino.rep)
          ocorrencias.teste<-rbind(ocorrencias.teste,ocorrencias.teste.rep)
        }
        especies.treino<-unique(ocorrencias.treino.rep[,1])
      }
      ...
      for (g in 1:length(especies.treino)){
        ocorrencias.treino.sp<-ocorrencias.treino.rep[ocorrencias.treino.rep[,1]==especies.treino[g],]
        ocorrencias.teste.sp<-ocorrencias.teste.rep[ocorrencias.teste.rep[,1]==especies.treino[g],]
        print(as.character(especies.treino[g]))
        
        if(msdm!=0){
          if(msdm==1){
            msdm.lat<-raster(read.asc(paste(dir.msdm,list.files(dir.msdm,pattern=as.character(especies[g])),sep="\\")[1]))
            msdm.long<-raster(read.asc(paste(dir.msdm,list.files(dir.msdm,pattern=as.character(especies[g])),sep="\\")[2]))
            ...
            msdm.ras<-raster(read.asc(paste(dir.msdm,paste(as.character(especies[g]),".asc",sep=""),sep="\\")))
            names(msdm.ras)<-"Dist"
            environmental<-stack(environmental.t,msdm.ras)
            env.env<-rasterToPoints(environmental)
            ocorrencias.tr.var<-extract(environmental,ocorrencias.treino.sp[,2:3],cellnumber=F)
        ...
        pb.0<-rep(0,round(((nrow(ocorrencias.treino.sp)-(Prev*nrow(ocorrencias.treino.sp)))/Prev)))
        set.seed(g)
        ...
        input.occ.back<-rbind(ocorrencias.treino.max,background.env.max)
        input.occ.back<-input.occ.back[,-c(1:3)]
        #print(paste("Pre-NArm.....",nrow(input.occ.back),sep=""))
        input.occ.back<-na.omit(input.occ.back)
        #print(paste("Pos-NArm.....",nrow(input.occ.back),sep=""))
        row.names(input.occ.back)<-NULL
        
        if(transf=="random"){
          
          msk.even<-raster(read.asc(paste(dir.msk,list.files(dir.msk,pattern=as.character(especies[g])),sep="\\")[1]))
          msk.odd<-raster(read.asc(paste(dir.msk,list.files(dir.msk,pattern=as.character(especies[g])),sep="\\")[2]))
          msk<-merge(msk.even,msk.odd)
          rm(msk.even)
          rm(msk.odd)
          environmental<-mask(environmental.t,msk)
          plot(environmental[[1]])
          points(ocorrencias.treino.sp[,2:3])
          ...
          if(pca=="N"){
            transf.env<-PCA_env_TML(environmental)
            transf.env<-transf.env[[c(1,2)]]
          }else{
            transf.env<-environmental[[c(1,2)]]
          }
          res.sp<-Random_Moran_e_Schoener_TML(transf.env,ocorrencias.treino.sp,ocorrencias.teste.sp)
          res.sp<-cbind(as.character(especies.treino[g]),res.sp)
          res<-rbind(res,res.sp)
        ...
        if (any(algoritmo=='bioclim')){
          ...
          dir.b<-"BIO"
          if (file.exists(file.path(dir2, dir.b))){
            ...
            for (f in proj_prep){
              ...
              if(msdm!=0){
                if(msdm==1){
                  ...
                  msdm.ras<-raster(read.asc(paste(dir.msdm,list.files(dir.msdm,pattern=as.character(especies[g])),sep="\\")))
                  names(msdm.ras)<-"Dist"
                  proj<-stack(proj,msdm.ras)
                }
              }
              ...
              crs(Bioclim.sp)<-crs(environmental.t)
        ...
        if (any(algoritmo=='maxentS')){
          ...
          dir.mxs<-"MXS"
          if (file.exists(file.path(dir2, dir.mxs))){
          ...
          if (project=='S'){
            ...
            for (f in proj_prep){
              ...
              crs(Maxent.sp)<-crs(environmental.t)
        ...
        if (any(algoritmo=='maxentD')){
          ...
          dir.mxd<-"MXD"
          if (file.exists(file.path(dir2, dir.mxd))){
        ...
        if (any(algoritmo=='svm.reg')){
          ...
          dir.sv<-"SVR"
          if (file.exists(file.path(dir2, dir.sv))){
        ...
        if (any(algoritmo=='glm')){
          ...
          dir.gl<-"GLM"
          if (file.exists(file.path(dir2, dir.gl))){
          ...
          if (project=='S'){
            ...
            for (f in proj_prep){
              ...
              crs(GLM.sp)<-crs(environmental.t)
        ...
        if (any(algoritmo=='RF')){
          ...
          dir.rf<-"RDF"
          if (file.exists(file.path(dir2, dir.rf))){
        ...
        if (any(algoritmo=='MDA')){
          ...
          dir.mda<-"MDA"
          if (file.exists(file.path(dir2, dir.mda))){
          ...
          if (project=='S'){
            ...
            for (f in proj_prep){
              ...
              crs(MDA.sp)<-crs(environmental.t)
    ...
    setwd(dir2)
    parametros<-rbind(algoritmo,env,pca,project,Prev,particao,replicas,porcentagem,transf,msdm)
    colnames(parametros)<-NULL
    write.table(parametros,"Parametros.txt",sep="\t",col.names=F,row.names = c("Algoritmo","Env","PCA","Project","Prev","Particao","Replicas","Porcentagem","Transf","MSDM"))
    write.xlsx(ocorrencias.treino[,c(1:3)],paste("Ocorrencias_treino",".xls",sep=""),row.names=F)
    if(porcentagem!=1){
      write.xlsx(ocorrencias.teste[,c(1:3)],paste("Ocorrencias_teste",".xls",sep=""),row.names=F)      
    }
    if(transf=="random"){
      write.table(res,paste(dir2,"Moran_&_Schoener_Aleat.txt",sep="/"),row.names=F,sep="\t")
    }
  }
