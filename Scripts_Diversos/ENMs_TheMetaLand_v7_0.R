...
ENMs_TheMetaLand('C:\\Users\\decoa\\Desktop\\OdonatasBrasil\\Env',
                 env='asc',pca='N',project='N',Prev=0.5,particao="random",transf="N",
                 algoritmo=c("maxentS","RF","svm.reg"),msdm=0)
ENMs_TheMetaLand<-function(diretorio,env='',pca='',project='',Prev,particao,transf,algoritmo='',msdm){
  ...
  if (pca=="S"){
    environmental.t<-PCA_env_TML(environmental.t,diretorio)
  }
  
  if (project=='S'){
    print("Select the folder containing GCM folders:")
    dir.project<-choose.dir(getwd())
  ...
  if (particao=="k.fold"){
    ...
    for (e in 1:length(lista.treino)){
      ...
      pb.0<-rep(0,round(((nrow(pseudo.ausencias)-(Prev*nrow(pseudo.ausencias)))/Prev)))
