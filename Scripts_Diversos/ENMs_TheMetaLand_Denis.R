ENMs_TheMetaLand<-function (diretorio.input,diretorio.output,env='',pca='',background,k.fold,algoritmo=''){
  
  # Essa ? a função padronizada para cria?ao de modelos de distribui?ao do laborat?rio
  #TheMetaLand
  #Par?metro iniciais:
...
ENMs_TheMetaLand(paste(getwd()),
                 diretorio.output=
                     "/media/denis/Dados/Bibliotecas/Documents/My R scripts/Leanning R/SDM_with_R/Modelagem_Porf_andre/SDM_Alouatta/",
                 'asc',pca='N',10000,2,algoritmo=c('bioclim','svm','maxent','glm'))


ENMs_TheMetaLand(paste(getwd()),'asc',pca='N',10000,2,
                 algoritmo=c('maxent'))

# rm(list=ls())


# Prancha com todos os modelos...
setwd(diretorio.output)
Alouatta.sdms<-list.files(pattern='.asc')
Alouatta.sdms

# list.modelos<-c('bioclim','svm','maxent','glm')
# list.modelos

X11()
par(mfrow=c(2,4))
for(i in Alouatta.sdms[1:8]){
    plot(raster(read.asc(i)))
    title(paste(substr(i,22,nchar(i))))
}
savePlot(paste("SDMs-Alouatta.",substr(i,10,16),sep=""),type="tiff",device = dev.cur())


X11()
par(mfrow=c(2,4))
for(i in Alouatta.sdms[9:16]){
    plot(raster(read.asc(i)))
    title(paste(substr(i,20,nchar(i))))
}
savePlot(paste("SDMs-Alouatta.",substr(i,10,16),sep=""),type="tiff",device = dev.cur())
...
X11()
par(mfrow=c(2,4))
for(i in Alouatta.sdms[17:24]){
    plot(raster(read.asc(i)))
    title(paste(substr(i,21,nchar(i))))
}
savePlot(paste("SDMs-Alouatta.",substr(i,10,16),sep=""),type="tiff",device = dev.cur())
