ENMs_TheMetaLand<-function(diretorio,env='',pca='',project='',background,particao,algoritmo=''){
  ...
  #Essa e a funcao padronizada para criacao de modelos de distribuicao do laboratorio
  #TheMetaLand
  #Parametro iniciais:
    ...
    #project: O modelo será projetado em uma área diferente da qual foi criado? (S/N)
    #background: Numero de pontos utilizados para criar o background
    #particao: Tipo de particao de dados (bootstrap) --> k.fold não está funcionando bem
    #Algoritmos: os algoritmos utilizados para criacao dos modelos (GLM,SVM,MaxEnt,Bioclim)
      #bioclim : Bioclim
      #glm : GLM
      #svm : SVM
      #maxentS : Maxent Simples (Linear e Quadrático)
      #maxentD : Maxent Default (Todos os parâmetros)
      #RF : Random Forest
 ...
 ######### OBSERVACOES IMPORTANTES!!!!!!!!!!!!!!! #############
    ...
    ######## O formato das ocorrencias deve ser .txt e com as colunas: Species,Long,Lat, nessa ordem! ########
...
#1.VARIAVEIS AMBIENTAIS----
  ...
  #1.1.Realizar a PCA nas camadas ambientais(utilizar PCA como input)
    ...
    #1.3.Projeção
    if (project=='S'){
      print("Selecione o diretório com as camadas ambientais para projeção")
      dir_project<-choose.dir()
      setwd(dir_project)
      proj_prep<-list.files(pattern='.asc')
      proj_base<-read.asc(proj_prep[1])
      projection<-raster(proj_base)
      camadas.proj<-proj_prep[-1]
      
      for (a in 1:length(camadas.proj)){
        asc<-read.asc(camadas.proj[a])
        raster<-raster(asc)
        projection<-stack(projection,raster)
      }
      names(projection)<-substr(proj_prep,1,nchar(proj_prep)-4)
      projection.env<-rasterToPoints(projection)
    }else{
      projection<-environmental
      projection.env<-background.env
    }
...
#3. Criar os modelos:
  ...
  if (any(algoritmo=='mahal')){
    print("Select the folder to save Mahalanobis results")
    dir.mahal<-choose.dir()
  }

#3.2.Particao Bootstrap
...
if (particao =="bootstrap"){
  ...
  for (y in 1:replicas){
    ...
    if (porcentagem==1){
      for (x in 1:length(especies)){
        ...
        if (nrow(ocorrencias.unicas.especie)<5){
          print(paste("Ocorrencias de ",especies[x]," insuficientes para criar um modelo",sep=""))
        }else{
      ...
      write.table(ocorrencias.treino[,c(1:3)],paste("Ocorrencias_treino",y,".txt",sep=""),row.names=F,sep="\t")
    ...
    write.table(ocorrencias.treino[,c(1:3)],paste("Ocorrencias_treino",y,".txt",sep=""),row.names=F,sep="\t")
    write.table(ocorrencias.teste[,c(1:3)],paste("Ocorrencias_teste",y,".txt",sep=""),row.names=F,sep="\t")
    
    especies.treino<-unique(ocorrencias.treino[,1])
    }
    
#3.3.Criar os Modelos
      ...
      if (any(algoritmo=='bioclim')){
        ...
        for (g in 1:length(especies.treino)){
          ...
          Bioclim.sp<-predict(Bioclim.modelo,projection)
      ...
      if (any(algoritmo=='mahal')){
       ...
       setwd(dir.mahal)
        ...
        for (g in 1:length(especies.treino)){
          ...
          Mahal.sp<-predict(Mahal.modelo,projection)
      ...
      if (any(algoritmo=='maxentS')){
        ...
        for (h in 1:length(especies.treino)){
          ...
          Maxent.sp<-predict(Maxent.modelo,projection)
          writeRaster(Maxent.sp,paste(especies.treino[h],'Simples.asc',sep=''),format='ascii')
        }
      }
      ...
      if (any(algoritmo=='maxentD')){
        ...
        for (h in 1:length(especies.treino)){
          ...
          Maxent.sp.default<-predict(Maxent.default,projection)
          writeRaster(Maxent.sp.default,paste(especies.treino[h],'Default.asc',sep=''),format='ascii')
        }
      }
      ...
      if (any(algoritmo=='svm')){
        ...
        for (i in 1:length(especies.treino)){
          ...
          SVM.sp<-predict(projection,svm.modelo)
      ...
      if (any(algoritmo=='glm')){
        ...
        for (j in 1:length(especies.treino)){
          ...
          GLM.sp<-predict(projection,glm.modelo)
    ...
    if (any(algoritmo=='RF')){
      ...
      for (k in 1:length(especies.treino)){
        ocorrencias.treino.sp<-ocorrencias.treino[ocorrencias.treino[,1]==especies.treino[k],]
        
        #Agrupar background e presencas
        pb.1<-rep(1,nrow(ocorrencias.treino.sp))
        ocorrencias.treino.max<-cbind(ocorrencias.treino.sp,pb.1)
        ocorrencias.treino.max<-ocorrencias.treino.max[,-4]
        species.pb<-rep(especies.treino[k],nrow(ocorrencias.treino.max))
        pb.0<-rep(0,nrow(ocorrencias.treino.max))
        background.sample<-sample(1:nrow(background.env),length(pb.0))
        background.sample<-background.env[background.sample,]
        background.env.max<-as.data.frame(cbind(background.sample,pb.0))
        ...
        RF.sp<-predict(object=RF.modelo,newdata=projection.env,type="response")
        RF.xy<-data.frame(cbind(background.env[,1:2],RF.sp))
        gridded(RF.xy)<-~x+y
        RF.raster<-raster(RF.xy)
        writeRaster(RF.raster,paste(especies.treino[k],'.asc',sep=''),format='ascii')
      }
    }
    ...
    if (any(algoritmo=='MDA')){
      ...
      for (m in 1:length(especies.treino)){
        ocorrencias.treino.sp<-ocorrencias.treino[ocorrencias.treino[,1]==especies.treino[m],]
        
        #Agrupar background e presencas
        pb.1<-rep(1,nrow(ocorrencias.treino.sp))
        ocorrencias.treino.max<-cbind(ocorrencias.treino.sp,pb.1)
        ocorrencias.treino.max<-ocorrencias.treino.max[,-4]
        species.pb<-rep(especies.treino[m],nrow(ocorrencias.treino.max))
        pb.0<-rep(0,nrow(ocorrencias.treino.max))
        background.sample<-sample(1:nrow(background.env),length(pb.0))
        background.sample<-background.env[background.sample,]
        background.env.max<-as.data.frame(cbind(background.sample,pb.0))
        ...
        MDA.sp<-predict(projection,MDA.modelo)
        
        writeRaster(RF.raster,paste(especies.treino[m],'.asc',sep=''),format='ascii')
      }
    }
...
ENMs_TheMetaLand('C:\\Users\\Andre\\Google Drive\\Mestrado\\Poliquetas_Pacifico\\Rotated Pacific','asc',pca='N',project='S',10000,"bootstrap",algoritmo=c('RF','glm','svm','MDA'))
