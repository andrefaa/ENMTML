ENMs_TheMetaLand('C:\\Users\\decoa\\Desktop\\Andre\\pcaflow',env='asc',pca='N',project='N',particao="random",transf="lat.band",algoritmo='maxentS',msdm=0)

ENMs_TheMetaLand<-function(diretorio,env='',pca='',project='',particao,transf,algoritmo='',msdm){
 ...
 ## OBSERVACOES IMPORTANTES!!!!!!!!!!!!!!!:
  ...
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\PCA_env_TML.R")
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\Optimal_Quadrants_Selection_TML.R")
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\Occ_Unicas_TML.R")
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\Random_Moran_e_Schoener_TML.R")
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\niche.overlap.functions.R")
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\occ.prep.functions.R")
  source("C:\\R\\Codigos\\Scripts_for_ENM_TheMetaLand\\Moran_for_Quadrants.R")
...
#2.ARQUIVOS DE OCORRENCIA----
  ...
  print("Select a folder to save model results:")
...
if(transf=="lat.band"){
  ...
  if(length(list.files(dir.transf)) !=0){
    print("Choose directory with the latitudinal bands:")
    dir.opt<-choose.dir(getwd())
  }else{
    if(pca=="N"){
  ...
  for (x in especies){
    ...
    for (y in 1:2){
      ...
      names(ext.t[[-y]])<-names(ext)[[y]]
      transf.msk<-stack(mask(transf.env,ext[[y]]),ext[[y]])
      proj.msk<-stack(mask(transf.env,ext[[-y]]),ext[[-y]])
      ...
      writeRaster(Maxent.transf,paste(dir.transf,paste(x,names(ext.t[[y]]),"-",names(ext.t[[-y]]),'.asc',sep=''),sep="/"),format='ascii',overwrite=T)
      }
    }
  }
...
#3.3.Criar os modelo de distribuicao
  ...
  for (g in 1:length(especies.treino)){
    ...
    ocorrencias.teste.sp<-ocorrencias.teste[ocorrencias.teste[,1]==especies.treino[g],]
    ...
    if (any(algoritmo=='bioclim')){
      ...
      if (project=='S'){
        ...
        for (f in proj_prep){
          ...
          if (msdm==1){
            for (b in 1:length(list.msdm)){
              msdm.asc<-read.asc(paste(dir.msdm,list.msdm[b],sep="\\"))
              msdm.ras<-raster(msdm.asc)
              proj<-stack(proj,msdm.ras)
            }
          }
          
          if(msdm==2|| msdm==3|| msdm==4){
            msdm.asc<-read.asc(paste(dir.msdm,list.msdm[g],sep="/"))
            msdm.ras<-raster(msdm.asc)
            proj<-stack(proj,msdm.ras)
    ...
    if (any(algoritmo=='svm.cls')){
      ...
      if (project=='S'){
        ...
        env.svm<-rasterToPoints(environmental)
        novdata<-as.matrix(env.svm[,-c(1,2)])
        SVM.sp<-kernlab::predict(object=svm.modelo,newdata=novdata,type="probabilities")
        SVM.sp<-as.data.frame(cbind(project.env[,1:2],SVM.sp[,2]))
        colnames(SVM.sp)<-c("x","y","Adeq")
        gridded(SVM.sp)<-~ x+y
    ...
    if (any(algoritmo=='RF')){
      ...
      if (project=='S'){
        ...
        for (f in proj_prep){
          ...
          writeRaster(RF.raster,paste(f,especies.treino[g],'.asc',sep=''),format='ascii',overwrite=T)
        ...
        gridded(RF.xy)<-~x+y
        RF.raster<-raster(RF.xy)
        writeRaster(RF.raster,paste("PRES_",especies.treino[g],'.asc',sep=''),format='ascii',overwrite=T)
        ...
        gridded(RF.xy)<-~x+y
        RF.raster<-raster(RF.xy)
        writeRaster(RF.raster,paste(especies.treino[g],'.asc',sep=''),format='ascii',overwrite=T)
