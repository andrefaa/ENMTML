ENMs_TheMetaLand('C:\\Users\\decoa\\Desktop\\Variaveis_Laila\\Input','asc',pca='N',project='N',"bootstrap",algoritmo=c('svm.cls','svm.reg','maxentS'),msdm=0)
...
if (particao =="bootstrap"){
  ...
  if (msdm==1){
    ...
    for (a in 1:length(list.msdm)){
      ...
      project.t<-stack(project.t,msdm.ras)
    }
    environmental<-environmental.t
    project<-project.t
...
#3.3.Criar os modelo de distribuicao
  ...
  for (g in 1:length(especies.treino)){
    ...
    if (any(algoritmo=='svm.reg')){
      ...
      SVM.sp<-predict(project,svm.modelo,type = "response")
