ENMs_TheMetaLand('C:\\Users\\decoa\\Desktop\\Disciplinas 2016_2\\Macroecologia\\Projeto\\Env\\PCA',env='asc',pca='N',project='N',Prev=0.5,particao="random",transf="N",algoritmo=c("RF"),msdm=0)
...
#1.VARIAVEIS AMBIENTAIS----
  ...
  if(env == 'asc'){
    ...
    names(environmental.t)<-substr(env_prep,1,nchar(env_prep)-4)
    env.env<-rasterToPoints(environmental.t)
    rm(list=c("asc","asc_base","raster","env_prep","camadas"))
  }
...
if(transf=="lat.band"){
  ...
  if(length(list.files(dir.out)) !=0){
    ...
    if(pca=="N"){
      transf.env<-PCA_env_TML(environmental.t,diretorio)
...
#3.3.Criar os modelo de distribuicao
  ...
  for (g in 1:length(especies.treino)){
    ...
    if(transf=="random"){
      ...
      if(pca=="N"){
        transf.env<-PCA_env_TML(environmental,diretorio)
        transf.env<-transf.env[[c(1,2)]]
      }else{
