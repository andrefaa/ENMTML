PCA_env_TMLA<-function(env,Dir){
  ...
  write.table(coef, file.path(DirPCATab,"Coefficients.txt"),sep="\t",row.names=F)
  vars<- env$model$sdev^2
  vars<- vars/sum(vars)
  write.table(data.frame(Axis=names(vars),Variance=round(vars,digits=10)), file.path(DirPCATab,"CumulativeVariance.txt"),sep="\t",row.names=F)
