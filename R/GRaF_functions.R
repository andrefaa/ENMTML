  ...
  function(x, vars = NULL, resolution = 50, CI = 0.95, prior = FALSE, data = TRUE, jitter = 1,
    ...
    for (i in vars) {
      ...
      if(i %in% facs) {
        ...
        # plot CIs
        graphics::rect(xleft = x.plot - width, ybottom = low, xright = x.plot + width, ytop = up, col = 'light grey',
             border = NA)
        # plot mean
        for(j in 1:m) {
          lines(x = c(x.plot[j] - width, x.plot[j] + width), y = rep(mn[j], 2), col = grDevices::rgb(0.4, 0.4, 0.4), lwd = 2)
        }

        if (prior) {
          xs <- sort(x.plot)
          xs[c(1, m)] <- c(-1, m + 1)
          lines(x$mnfun(predX) ~ xs, lty = 2, col = grDevices::rgb(0.4, 0.4, 0.4))
        }

        if (data) {
          graphics::rug(jitter(x$x[x$obsy == 1, i], jitter), side = 3, col = 'dark grey')
          graphics::rug(jitter(x$x[x$obsy == 0, i], jitter), side = 1, col = 'dark grey')
          graphics::box()
        }
        ...
        plot(mn ~ seqx, ylim = c(0, 1), type = 'n', xlab = nam,
             ylab = 'probability of presence', ...)
        graphics::polygon(x = c(seqx, rev(seqx)), y = c(up, rev(low)), col = 'light grey',
                border = NA)
        lines(mn ~ seqx, col = grDevices::rgb(0.4, 0.4, 0.4), lwd = 2)
        if (prior) {
          lines(x$mnfun(predX) ~ seqx, lty = 2, col = 'dark grey')
        }
        if (data) {
          graphics::rug(jitter(x$obsx[x$obsy == 1, i], jitter), side = 3, col = 'dark grey')
          graphics::rug(jitter(x$obsx[x$obsy == 0, i], jitter), side = 1, col = 'dark grey')
          graphics::box()
        }
      }
    }
