...
FitENM_TMLA_OLD <- function(RecordsData,
                   Variables,
                   Fut=NULL,
                   Part,
                   Algorithm,
  ...
  if(is.null(repl)==F){
    VALNAME <- paste('Validation','_',sep="",repl,'.txt' )
  }else{
  ...
  for(s in 1:length(SpNames)){
    ...
    if (any(Algorithm == "BIO")) {
      ...
      Eval <- list()
      for (i in 1:N) {
        PredPoint <- predict(Model[[i]], PAtest[[i]][, VarCol])
      ...
      if(Save=="Y"){
        RastPart <- list()
        for(i in 1:N){
          RastPart[[i]] <- predict(Variables, Model[[i]])
          if(N!=1){
      ...
      Eval <- dismo::evaluate(PredPoint[PredPoint$PresAbse == 1, 2],
                       PredPoint[PredPoint$PresAbse == 0, 2])
      Validation[, 'THR_FULL'] <- as.numeric(threshold(Eval)[Threshold])
      Validation_BIO[[s]] <- data.frame(Sp=SpNames[s], Algorithm="BIO", Validation)
      ListRaster[["BIO"]] <- predict(Variables, Model)
      if(is.null(Fut)==F){
    ...
    if(any(Algorithm == 'MXD')){
      ...
      Eval <- list()
      for (i in 1:N) {
        PredPoint <- c(predict(Model[[i]], PAtestM[[i]][, VarCol], clamp=F, type="cloglog"))
      ...
      PredPoint <- c(predict(Model, SpDataM[, VarCol], clamp=F, type="cloglog"))
      ...
      ListRaster[["MXD"]] <- predict(Variables, Model, clamp=F, type="cloglog")
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
          ListFut[[ProjN[k]]][["MXD"]] <- predict(VariablesP[[k]], Model)
        }
      }
    }
    ...
    if(any(Algorithm == 'MXS')){
      ...
      ListRaster[["MXS"]] <- predict(Variables, Model,type="cloglog")
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
          ListFut[[ProjN[k]]][["MXS"]] <- predict(VariablesP[[k]], Model)
        }
      }
    }
    ...
    if(any(Algorithm == 'MLK')) {
      ...
      ListRaster[["MLK"]]<- predict(Variables, Model)
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
    ...
    if (any(Algorithm == "SVM")) {
      ...
      ListRaster[["SVM"]] <- predict(Variables, Model)
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
    ...
    if (any(Algorithm == "RDF")) {
      ...
      ListRaster[["RDF"]] <- predict(Variables, Model)
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
    ...
    if(any(Algorithm == 'GAM')) {
      ...
      Eval <- list()
      for (i in 1:N) {
        PredPoint <- c(predict(Model[[i]], PAtest[[i]][, VarCol]))
        PredPoint <- data.frame(PresAbse = PAtest[[i]][, "PresAbse"], PredPoint)
        Eval[[i]] <- dismo::evaluate(PredPoint[PredPoint$PresAbse == 1, 2],
                              PredPoint[PredPoint$PresAbse == 0, 2])
      }
      ...
      PredPoint <- c(predict(Model, SpData[, VarCol]))
      PredPoint <- data.frame(PresAbse = SpData[, "PresAbse"], PredPoint)
      Eval <- dismo::evaluate(PredPoint[PredPoint$PresAbse == 1, 2],
                       PredPoint[PredPoint$PresAbse == 0, 2])
      Validation[, 'THR_FULL'] <- as.numeric(threshold(Eval)[Threshold])
      Validation_GAM[[s]] <- data.frame(Sp = SpNames[s], Algorithm = "GAM", Validation)
      ListRaster[["GAM"]] <- predict(Variables, Model)
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
    ...
    if(any(Algorithm == 'GLM')) {
      ...
      ListRaster[["GLM"]] <- predict(Variables, Model)
      if(is.null(Fut)==F){
        for(k in 1:length(VariablesP)){
    ...
    for (i in 1:N) {
      PredPoint <- predict(Model[[i]], PAtest[[i]][, VarCol])
      PredPoint <- data.frame(PresAbse = PAtest[[i]][, "PresAbse"], PredPoint)
      Eval[[i]] <- dismo::evaluate(PredPoint[PredPoint$PresAbse == 1, 'posterior.mode'],
                            PredPoint[PredPoint$PresAbse == 0, 'posterior.mode'])
    }
    #GAU threshold
    Thr <- unlist(sapply(Eval, function(x) threshold(x)[Threshold]))
    names(Thr) <- Threshold
    ...
    Pred <- predict.graf.raster(Model, Variables, type = "response", 
                                CI = 0.95, maxn = NULL)
    ListRaster[["GAU"]] <- Pred$posterior.mode
    if(is.null(Fut)==F){
      for(k in 1:length(VariablesP)){
        ListFut[[ProjN[k]]][["GAU"]] <- predict(VariablesP[[k]], Model)
      }
    }
    }
    ...
    # Without Ensemble
      ...
      ListRaster2 <- STANDAR(brick(ListRaster))
      for(i in 1:nlayers(ListRaster)){
        writeRaster(round(ListRaster2[[i]], 4), 
                    paste(folders[i], '/',SpNames2[s],".tif", sep=""),
                    format='GTiff',
                    overwrite=TRUE)
        Thr <- SpValidation[SpValidation$Algorithm==names(ListRaster[i]), 'THR_FULL']
    ...
    if(is.null(Fut)==F){
      for(p in 1:length(ListFut)){
        for(o in 1:length(ListFut[[p]])){
          Thr <- SpValidation[SpValidation$Algorithm==names(ListFut[[p]])[o], 'THR_FULL']
    ...
    if(any(PredictType=="Mean")){
      ...
      Validation_Mean[[s]] <- data.frame(Sp=SpNames[s], Algorithm="MEA", Validation)
      # Presente threshold used to future prediction
      tmp <- data.frame(Species=SpNames[s],Thr)
      ThresholdPresent[[s]] <- tmp
      
      writeRaster(Final, 
                  paste(DirMean, '/',SpNames2[s],".tif", sep=""),
                  format='GTiff',
                  overwrite=TRUE)
      TYPE <- Validation_Mean[[s]]["TYPE"]
      for(h in 1:length(Thr)){
    ...
    if(any(PredictType=='Sup')){
      ...
      Validation_Sup[[s]] <- data.frame(Sp=SpNames[s], Algorithm="SUP", Validation)
    }
    ...
    if (any(PredictType == 'PCA')) {
      ...
      names(Thr) <- Threshold
      Validation <- SUMMRES(list(Eval),N=1,Thr)
      Validation_PCA[[s]] <- data.frame(Sp=SpNames[s], Algorithm="PCA", Validation)
      # Presente threshold used to future prediction
      tmp <- data.frame(Species=SpNames[s],Thr)
      ThresholdPresent[[s]] <- tmp
    ...
    if (any(PredictType == 'PCASup')) {
      ...
      for(h in 1:length(Nom)){
        ...
        writeRaster(Final, 
                    paste(DirPCASup, '/',SpNames2[s],"_",Nom[h],".tif", sep=""),
                    format='GTiff',
                    overwrite=TRUE)
        writeRaster(Final>=as.numeric(Thr), 
                    paste(DirPCASupCat, '/',SpNames2[s],"_",Nom[h],".tif", sep=""),
                    format='GTiff',
                    overwrite=TRUE)
        if(is.null(Fut)==F){
          for(p in 1:length(ListFut)){
            ...
            writeRaster(Final, 
                        file.path(ModFut[p],"ENS","PCASup",paste(SpNames2[s],Nom[h],sep="_")),
                        format='GTiff',
                        overwrite=TRUE)
            writeRaster(Final>=as.numeric(Thr), 
                        file.path(ModFut[p],"ENS","PCASup","BIN",paste(SpNames2[s],Nom[h],sep="_")),
                        format='GTiff',
                        overwrite=TRUE)
          }
        }
      }
      Validation_PCASup[[s]] <- data.frame(Sp=SpNames[s], Algorithm="PCS", Validation)
    }
    ...
    if (any(PredictType == 'PCAThr')) {
      ...
      for(n in 1:length(TYPE)){
       ...
       SpValType <-SpValidation[SpValidation$TYPE==TYPE[n],]  
        ...
        Validation_PCAThr[[s]] <- data.frame(Sp=SpNames[s], Algorithm="PCT", Validation)
        # Presente threshold used to future prediction
        tmp <- data.frame(Species=SpNames[s],Thr)
        ThresholdPresent[[s]] <- tmp
        
        writeRaster(Final, 
                    paste(DirPCAThr, '/',paste(SpNames2[s],TYPE[n],sep="_"),".tif", sep=""),
                    format='GTiff',
                    overwrite=TRUE)
        writeRaster(Final>=unlist(Thr), 
                      paste(DirPCAThrCat, '/',SpNames2[s],"_",TYPE[n],".tif", sep=""),
                      format='GTiff',
                      overwrite=TRUE)
      }
      if(is.null(Fut)==F){
        for(p in 1:length(ListFut)){
          for(n in 1:length(TYPE)){
            ...
            writeRaster(Final, 
                        file.path(ModFut[p],"ENS","PCAThr",paste(SpNames2[s],TYPE[n],sep="_")),
                        format='GTiff',
                        overwrite=TRUE)
            for(h in 1:length(Thr)){
              writeRaster(Final>=unlist(Thr), 
                          file.path(ModFut[p],"ENS","PCAThr","BIN",paste(SpNames2[s],TYPE[n],sep="_")),
                          format='GTiff',
                          overwrite=TRUE)
            }
          }
