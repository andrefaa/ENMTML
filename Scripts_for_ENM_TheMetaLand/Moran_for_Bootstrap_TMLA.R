...
#Moran_for_Quadrants <- function(occurrence,pc1,m){
  #Parametros:
    ...
    for(i in spN){
      ...
      #Moran Correction
      sp.dists <- 1/(as.matrix(dist(cbind(occSP$x, occSP$y))))
      diag(sp.dists) <- 0
      ...
      for (a in 1:nrow(sp.dists.even)){
        sp.dists.even[,a]<-ifelse(sp.dists.even[,a]==sp.dists.even[maximo.colO[a],a],sp.dists.even[maximo.colO[a],a],0)
      }
      
      for (a in 1:nrow(sp.dists.odd)){
        sp.dists.odd[,a]<-ifelse(sp.dists.odd[,a]==sp.dists.odd[maximo.colE[a],a],sp.dists.odd[maximo.colO[a],a],0)
      }
      
      sp.dists<-pmax(sp.dists.odd,sp.dists.even)                                 
      
      Moran[[i]]<-as.numeric(Moran.I(occSP$occs.z,sp.dists,scaled=T)[1])
    }
